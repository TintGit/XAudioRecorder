...
public class XTimer {
    ...
    public typealias XTimerHandler = (XTimer) -> Void

    private var handler: XTimerHandler

    public init(interval: DispatchTimeInterval, repeats: Bool = false, leeway: DispatchTimeInterval = .seconds(0), queue: DispatchQueue = .main , handler: @escaping XTimerHandler) {
    ...
    public static func repeaticTimer(interval: DispatchTimeInterval, leeway: DispatchTimeInterval = .seconds(0), queue: DispatchQueue = .main , handler: @escaping XTimerHandler ) -> XTimer {
        return XTimer(interval: interval, repeats: true, leeway: leeway, queue: queue, handler: handler)
    }
    ...
    public func rescheduleHandler(handler: @escaping XTimerHandler) {
        self.handler = handler
        internalTimer.setEventHandler { [weak self] in
            if let strongSelf = self {
                handler(strongSelf)
...
public class XCountDownTimer {
    ...
    private let internalTimer: XTimer

    private var leftTimes: Int

    private let originalTimes: Int

    private let handler: (XCountDownTimer, _ leftTimes: Int) -> Void

    public init(interval: DispatchTimeInterval, times: Int,queue: DispatchQueue = .main , handler:  @escaping (XCountDownTimer, _ leftTimes: Int) -> Void ) {

        self.leftTimes = times
        self.originalTimes = times
        self.handler = handler
        self.internalTimer = XTimer.repeaticTimer(interval: interval, queue: queue, handler: { _ in
        })
